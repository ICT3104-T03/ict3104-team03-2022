...
videodir = ".dataset/ava/ava_videos/"    # TODO: put the path to your AVA dataset here
...
progress_bar_instance = progress_bar(len(videonames))
...
for video_id in videonames:
    ...
    for mid in range(900, 1800):
        ...
        if not os.path.isdir(os.path.join(clips_dir, "{:05d}".format(mid))):
            ...
            ffmpeg_command = 'ffmpeg -ss {} -i {} -qscale:v 4 -vf scale=-1:360 -t {} {}/%05d.jpg -hide_banner -loglevel error'.format(mid, videofile, clip_length, os.path.join(clips_dir, "{:05d}".format(mid)))
