...
if __name__ == '__main__':
    ...
    gt_file = './data/full_charades.json'
    classes = 157
    ...
    for vid in gt_new.keys():
        ...
        new_apm.add(logit, gt_new[vid])
        print('Saving accuracy...')
        item_index = 0
        for ann in prev_gt[vid]['actions']:
            test = np.array(new_apm.value())
            print(vid, new_apm.value())
            act_index = ann[0]
            if(test[int(act_index)]):
              prev_gt[vid]['actions'][item_index].append(round(test[int(act_index)]*100,2))
              item_index += 1
        caption_result[vid] = prev_gt[vid]
        new_apm.reset()
        ...
        print(args.pkl_path)
    with open('./annotations/json_data.json', 'w') as outfile:
      json.dump(caption_result,outfile)
    # per-frame mAP
